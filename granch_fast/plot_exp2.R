...
mod <- read_csv(file.path(GF,"exp2_infant_corrected.csv"), show_col_types=FALSE)     # corrected raw samples
...
d6 <- bind_rows(
  ...
  mod %>% transmute(trial_type, value="RANCH: corrected", LT, lo=NA, hi=NA)
...
p6 <- ggplot(d6, aes(trial_type, LT, color=value, group=value)) +
  ...
  scale_color_manual(values=c("Infant behaviour"="black","RANCH: grid (original)"="#b58900","RANCH: corrected"="#2aa198"), name="") +
...
ggsave(file.path(FIGS,"fig6_infant_exp2_compare.png"), p6, width=8, height=5, dpi=150, bg="white")
...
modA <- read_csv(file.path(GF,"exp2_adult_corrected.csv"), show_col_types=FALSE)
...
p7 <- plot_grid(
  ...
  panel(modA, "RANCH: corrected")+theme(legend.position=c(0.7,0.8), legend.key.size=unit(.35,"cm")),
  nrow=1)
ggsave(file.path(FIGS,"fig7_adult_exp2_compare.png"), p7, width=15, height=4.4, dpi=150, bg="white")
cat("wrote fig6_infant_exp2_compare.png and fig7_adult_exp2_compare.png\n")
