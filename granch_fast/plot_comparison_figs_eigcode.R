...
pc1 <- mk_corr(file.path(GF,"exp1_infant_eigcode.csv"),"RANCH: exact (paper EIG) (best fit)")+theme(legend.position="none")
pc2 <- mk_corr(file.path(GF,"exp1_infant_eigcode_tight.csv"),"RANCH: exact (paper EIG) (tighter concept)")+
  theme(legend.position=c(0.75,0.85), legend.key.size=unit(.35,"cm"))
ggsave(file.path(FIGS,"fig4_infant_exp1_eigcode.png"),
...
amod <- read_csv(file.path(GF,"exp1_adult_eigcode.csv"), show_col_types=FALSE) %>% rename(tt=trial_type)
...
pAc <- ggplot(amod, aes(trial_number,y,color=tt,group=tt)) + geom_point(size=1.8)+geom_line(linewidth=1) +
  ...
  theme_p()+labs(x="Trial number", y="Scaled samples", title="RANCH: exact (paper EIG)")+theme(legend.position=c(0.8,0.85))
ggsave(file.path(FIGS,"fig5_adult_exp1_eigcode.png"),
       plot_grid(pAb,pAg,pAc,nrow=1), width=15, height=4.4, dpi=150, bg="white")

cat("wrote fig4_infant_exp1_eigcode.png and fig5_adult_exp1_eigcode.png\n")
