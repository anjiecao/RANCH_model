...
import pandas as pd
import pickle
from . import init_model_tensor, init_stimuli_tensor, main_sim_tensor, init_params_tensor, compute_prob_tensor, lesioned_sim, proxy_sim
import gc
#import ipdb
    ...
    for i in range(pair_each_stim): 
        for c_type in all_complexity_type: 
            for s_type in all_background_blocks: 
                s = init_stimuli_tensor.granch_stimuli(n_feature, s_type)
        ...
        for i in range(pair_each_stim): 
            for v_type in all_violation_type: 
                for s_type in all_deviant_blocks: 
                    s = init_stimuli_tensor.granch_stimuli(1, s_type)
...
def sample_multiple_pair(pair_each_stim, distance_range = []):
    ...
    all_stimuli_info = []
    for i in range(pair_each_stim): 
        s1 = init_stimuli_tensor.granch_stimuli(1, 'BBBBBB')
        s2 = init_stimuli_tensor.granch_stimuli(1, 'BDBBBB')
        s3 = init_stimuli_tensor.granch_stimuli(1, 'BBBDBB')
        s4 = init_stimuli_tensor.granch_stimuli(1, 'BBBBBD')
