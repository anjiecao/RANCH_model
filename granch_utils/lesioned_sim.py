...
def granch_no_noise_simulation(params, model, stimuli): 

    no_noise_sim_epsilon = 0.0000000000000000000000000000001
    stimulus_idx = 0
    t = 0 # following python tradition we are using 0-indexed
    current_stim_t = 0
    while t < params.max_observation and stimulus_idx < stimuli.n_trial: 
        ...
        if model.current_t == 0 or (not model.if_same_stimulus_as_previous_t()): 
            model.update_possible_observations(no_noise_sim_epsilon, params.hypothetical_obs_grid_n)
        ...
        # update the noisy observation on current model stimulus 
        model.update_model_stimulus_id()
        model.update_noisy_observation(no_noise_sim_epsilon)
